...
async def permalink(mention):
    ...
    if user.id == 1970797144:
        return await edit_or_reply(mention, f"**- تاج راسك  مبرمج السورس  **")
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 1970797144:
        return await edit_or_reply(mention, f"**- متلعبش مع مبرمج السورس شادو يعبيط  **")
    iqth = user.first_name.replace("\u2060", "") if user.first_name else user.username
...
async def iq(mention):
    ...
    if not user:
        return
    if user.id == 1970797144:
        return await edit_or_reply(mention, f"**- مبرمج السورس  **")
    iqth = user.first_name.replace("\u2060", "") if user.first_name else user.username
...
async def gbun(event):
    ...
    if event.reply_to_msg_id:
        ...
        idd = reply_message.sender_id
        if idd == 1970797144:
            await catevent.edit(                "عذرا مبرمج السورس كلاينر"            )
        else:
