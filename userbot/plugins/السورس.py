...
IQTHONPC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/3d517769992e875de6096.mp4"
...
IQPIC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/3d517769992e875de6096.mp4"
...
async def iq(iqthonevent):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "𝐰𝐞𝐥𝐜𝐨𝐦𝐞 𝐬𝐨𝐮𝐫𝐜𝐞 𝐟𝐚𝐬𝐭𝐞𝐫 𓃠"
    IQTHON_IMG = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/3d517769992e875de6096.mp4"
...
.𓄌 : ping  {ping}  𓇡.
.𓄌 : version 7.5  𓇡.
.𓄌 : Source Faster : @FA9SH 𓇡."""
@iqthon.on(admin_cmd(pattern="رابط التنصيب(?: |$)(.*)"))    
async def source(e):
    await edit_or_reply(e, "https://github.com/UDFAST/TelethonArab",)
...
async def amireallyalive(event):
    ...
    IQTHON_ALIVE_TEXT = "❬ سورس فـاسـتـر - Source Faster ، 🦅  ❭ :"
...
async def _(event):
    ...
    OUTPUT = (f"🦅  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس فـاسـتـر @FA9SH:**\n\n{o}")
...
async def _(event):
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**🦅   تم تحديث سورس فـاسـتـر ↻**")
    sandy = await edit_or_reply(event , "🦅  جـاري تـحديـث سورس فـاسـتـر  🔄\n🔹 - قـد يستغـرق الأمـر 5 - 10 دقائـق انتـظـر\nلاتقـم بتحـديث أكثـر من 3 مـرات باليـوم" ,)
    try:
...
قناه المتغيرات او الفارات : @shadow49""")
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**🦅  📑 يرجى متابع قناه الفارات تجدها هنا : @FA9SH")
...
async def variable(var):
    ...
    if exe == "get":
        ...
        if variable in heroku_var:
            await ics.edit("**⌔ تم تغيـر** `{}` **:**\n **- المتغير :** `{}` \n**- يتم الان اعـادة تشغيـل بـوت فـاسـتـر يستغـرق الامر 2-1 دقيقـه ▬▭ ...**".format(variable, value))
        else:
...
async def iq(event):
    await edit_or_reply(event, """✐  ⦗ اوامـر سورس فـاسـتـر ⦘
                                             ┉┉┉┉┉┉×┉┉┉┉┉
〖`.م1`〗⏎  اوامر الحساب 1
〖`.م2`〗⏎  اوامر الحساب 2
〖`.م3`〗⏎  اوامر الحساب 3
